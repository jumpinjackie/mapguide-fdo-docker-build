...
"$DOCKER_CMD" run --rm -it -v ${ROOT}:/artifacts $CONTAINER_NAME cp -r /usr/local/src/mapguide/build/artifacts /artifacts
if [ "$?" -ne 0 ] ; then
    exit 1
fi
...
echo "$DOCKER_CMD run --rm -it $CONTAINER_NAME ls /usr/local/src/mapguide/build"
echo
